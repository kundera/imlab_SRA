...
class ProblemReader:
    ...
    def get_problem(self):
        ...
        cur = con.cursor()
        sql = 'SELECT aisleNum, columnNum, floorNum, itemTypeNum,  shuttleNum FROM problemConfigNew WHERE idx = '+str(self.table_idx)
        cur.execute(sql)
        row = cur.fetchone()
        p = problem.Problem(row[0], row[1], row[2], row[3], row[4])
